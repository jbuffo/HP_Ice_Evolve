function [T_np1,Conv_top,Conv_bottom,Ra,delta_thick_top,delta_thick_bottom,is_convect,k_bar,top_hold,bottom_hold]...
    =HP_Ice_Evolve_v9(T_n,k_i,rho_i,c_i,dt,dz,T_surf,Base_flux,Tm,TTol,P_n,phasenew,Ra_c,eta_0,alpha,g_s,A,Mx,Rx)
...
% Making corrections I talked about with Krista Soderlund:
%  - For Ra # caculation subtract the adiabatic profile
...
for i=1:length(top_hold)
    [T_ad,dT_dz_ad,P_ad,rho_ad,alpha_ad,Cp_ad,K_ad,phase_ad,g_s_ad,z_ad] = ...
                adiabat_profile(P_n(top_hold(i)),T_n(top_hold(i)),dz*(bottom_hold(i)-top_hold(i)+1),dz/1000,Mx,Rx);
    jj=1;
    for j=top_hold(i):bottom_hold(i)
        eta(j)=eta_0(j)*exp(A*((Tm(j)/T_np1_km1(j))-1));
        Ra(j)=(alpha(j)*(rho_i(j)^2)*c_i(j)*g_s*((T_np1_km1(bottom_hold(i))-T_ad(end))-(T_np1_km1(j)-T_ad(jj)))*((bottom_hold(i)-j)*dz)^3)/(k_i(j)*eta(j));
        if Ra(j)<0
            Ra(j)=0;
        else
        end
